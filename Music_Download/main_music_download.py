...
class MusicDownloadWindow(QWidget):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.settings = {
                "save_paths": {
                    "music": os.path.join(get_base_path(), "Songs"),
    ...
    def on_download_finished(self, file_path):
        ...
        try:
            ...
            try:
                ...
                # 从current_song获取歌曲信息
                current_song_dict = self.current_song if isinstance(self.current_song, dict) else {}
                duration = current_song_dict.get('duration', '00:00')
                # 确保duration是字符串格式
                ...
                # 构建原始格式的播放列表条目
                entry = {
                    'id': current_song_dict.get('id', ''),
                    'title': current_song_dict.get('title', ''),
                    'author': current_song_dict.get('author', '未知艺术家'),
                    'duration': duration,
                    'album': current_song_dict.get('album', '未知专辑'),
                    'url': current_song_dict.get('url', ''),
                    'pic': current_song_dict.get('pic', ''),
                    'lrc': current_song_dict.get('lrc', ''),
                    'path': file_path.replace('\\', '/'),
                    'filename': filename,
                    'extension': ext.lower(),
                    'file_size': file_stats.st_size if file_stats else 0,
                    'last_modified': file_stats.st_mtime if file_stats else 0
                }
                ...
                for it in playlist:
                    if isinstance(it, dict):
                        # 新格式使用'path'字段，旧格式使用'file'字段
                        it_path = it.get('path')
                        it_file = it.get('file')
                        entry_path = entry.get('path')
                        
                        # 如果it有'path'字段，直接比较
                        if it_path and entry_path and it_path == entry_path:
                            exists = True
                            break
                        # 如果it有'file'字段（相对路径），需要转换为绝对路径再比较
                        elif it_file and entry_path:
                            # 将相对路径转换为绝对路径
                            try:
                                abs_it_path = os.path.join(project_root, it_file)
                                abs_it_path = os.path.normpath(abs_it_path).replace('\\', '/')
                                if abs_it_path == entry_path:
                                    exists = True
                                    break
                            except:
                                pass
                ...
                if not exists:
                    ...
                    # 始终写入字典格式，与音乐播放器格式保持一致
                    playlist_data = {}
                    if os.path.exists(playlist_file):
                        try:
                            with open(playlist_file, "r", encoding="utf-8") as pf:
                                loaded = json.load(pf)
                                # 保留原始数据中的其他字段（如version, current_index, timestamp等）
                                if isinstance(loaded, dict):
                                    playlist_data = loaded
                                else:
                                    # 如果原始文件是列表，转换为字典格式
                                    playlist_data = {"playlist": loaded}
                        except Exception:
                            # 读取失败，创建新的字典
                            playlist_data = {}
                    ...
                    # 添加时间戳
                    import time
                    playlist_data["timestamp"] = time.time()
                    
                    # 添加当前索引（默认为-1）
                    if "current_index" not in playlist_data:
                        playlist_data["current_index"] = -1
                    
                    # 添加版本信息（可选）
                    if "version" not in playlist_data:
                        playlist_data["version"] = "1.0"
                    
                    # 写入文件
                    with open(playlist_file, "w", encoding="utf-8") as pfw:
                        json.dump(playlist_data, pfw, ensure_ascii=False, indent=2)
            except Exception:
