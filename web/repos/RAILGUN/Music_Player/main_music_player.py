...
class MusicPlayerWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        if not self.show_embedded_lyrics:
            self._window_width_without_lyrics = 1400 - 850
            self.setGeometry(100, 50, 1400 - 850, 800)
        else:
            self._window_width_without_lyrics = None
    ...
    def on_current_song_changed(self, song_info):
        if song_info:
            ...
            # 优先使用 song_info 中的 lyrics_path / cover_path 字段
    ...
    def on_toggle_lyrics_checkbox(self, state):
        ...
        try:
            ...
            if enabled:
                # 显示歌词
                self.right_panel.setFixedWidth(850)
                # 恢复窗口宽度
                ...
                # 缩小窗口
                new_width = self.width() - 850
                self.setGeometry(self.x(), self.y(), max(500, new_width), self.height())
